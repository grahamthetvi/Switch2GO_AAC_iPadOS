 ...
 * Switch2GOTextToSpeech to say the button text aloud and then perform an action based on the
 * subclass's implementation
 */
open class Switch2GOButton @JvmOverloads constructor(
    context: Context,
    ...
    protected open fun sayText(text: CharSequence?) {
        text?.let {
            Switch2GOTextToSpeech.speak(locale, it.toString())
    ...
    override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences?, key: String?) {
        when (key) {
            Switch2GOSharedPreferences.KEY_DWELL_TIME -> {
                dwellTime = sharedPrefs.getDwellTime()
            }
        }
    }
