...
class EyeGazeTrackingViewModel(
    ...
    private var sensitivity = Switch2GOSharedPreferences.DEFAULT_SENSITIVITY
    private var eyeGazeEnabled = false

    private val sharedPrefsListener =
        SharedPreferences.OnSharedPreferenceChangeListener { _, key ->
            when (key) {
                Switch2GOSharedPreferences.KEY_SENSITIVITY -> {
                    sensitivity = sharedPrefs.getSensitivity()
                }
                Switch2GOSharedPreferences.KEY_EYE_GAZE_ENABLED -> {
                    eyeGazeEnabled = sharedPrefs.getEyeGazeEnabled()
                }
                Switch2GOSharedPreferences.KEY_EYE_SELECTION -> {
                ...
                Switch2GOSharedPreferences.KEY_GAZE_AMPLIFICATION -> {
    ...
    private var gazeAmplification = Switch2GOSharedPreferences.DEFAULT_GAZE_AMPLIFICATION

    // Gaze amplification LiveData
    private val liveGazeAmplification = MutableLiveData(Switch2GOSharedPreferences.DEFAULT_GAZE_AMPLIFICATION)
